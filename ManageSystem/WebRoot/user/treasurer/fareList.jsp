...
<html>
  ...
  <body>
	...
	<h2 style="text-align:center;">调度费信息查询</h2><hr/>
	<c:if test="${sessionScope.admin!=null}">
	<a href="${pageContext.request.contextPath }/servlet/BasicFareListServlet?thispage=1" style="float: right;">调度费管理</a>
	</c:if>
	<c:if test="${sessionScope.clerk.type=='财务员'}">
	<div style="float:left;">
	<input type="button" id="print" onclick="window.location.href='${pageContext.request.contextPath }/servlet/PrintFareListServlet?from=1&&to=${requestScope.page.countpage}' " value="打印"/>
	</div><br/><br/>
	</c:if>
	...
	<table class="table table-striped center">
		...
		<c:forEach items="${requestScope.page.list }" var="fares">
			<tr>
				...
				<c:if test="${sessionScope.admin!=null}">
					<td><a href="${pageContext.request.contextPath }/servlet/FareListServlet?id=${fares.id}">删除</a></td>
				</c:if>
		  	</tr>
		</c:forEach>
	</table>
	...
	<a href="${pageContext.request.contextPath }/servlet/FareListServlet?thispage=${requestScope.page.firstpage}">首页</a>
	<a href="${pageContext.request.contextPath }/servlet/FareListServlet?thispage=${requestScope.page.prepage}">上一页</a>
	...
	<c:forEach begin="${begin }" end="${end }" step="1" var="i">
		...
		<c:if test="${requestScope.page.thispage != i }">
			<a href="${pageContext.request.contextPath }/servlet/FareListServlet?thispage=${i}">${i }</a>
		</c:if>
	</c:forEach>
	<!-- 分页逻辑结束 -->
	<a href="${pageContext.request.contextPath }/servlet/FareListServlet?thispage=${requestScope.page.nextpage}">下一页</a>
	<a href="${pageContext.request.contextPath }/servlet/FareListServlet?thispage=${requestScope.page.lastpage}">尾页</a>
	</div>
  </body>
</html>
