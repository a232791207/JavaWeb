...
<html>
  ...
  <body>
	...
	<h2 style="text-align:center;">审核订单</h2><hr/>
	<div style="float:left;">
	<input type="button" id="print" onclick="window.location.href='${pageContext.request.contextPath }/servlet/PrintOrderListForTreasurerServlet?from=1&&to=${requestScope.page.countpage}' " value="打印"/>
	</div><br/><br/>
	<table class="table table-striped center">
		...
		<c:forEach items="${requestScope.page.list }" var="order">
			<tr>
				...
				<td>
					<a href="${pageContext.request.contextPath }/servlet/UpdateOrderState?id=${order.orderid}&check=check&state=审核通过&format=${order.format}&level=${order.level}" id="pass">审核通过/</a>
					<a href="${pageContext.request.contextPath }/servlet/UpdateOrderState?id=${order.orderid}&check=check&state=审核未通过&format=${order.format}&level=${order.level}" id="unpass">审核未通过</a>
				</td>
		  	</tr>
		</c:forEach>
	</table>
	...
	<a href="${pageContext.request.contextPath }/servlet/OrderList2Servlet?thispage=${requestScope.page.firstpage}">首页</a>
	<a href="${pageContext.request.contextPath }/servlet/OrderList2Servlet?thispage=${requestScope.page.prepage}">上一页</a>
	...
	<c:forEach begin="${begin }" end="${end }" step="1" var="i">
		...
		<c:if test="${requestScope.page.thispage != i }">
			<a href="${pageContext.request.contextPath }/servlet/OrderList2Servlet?thispage=${i}">${i }</a>
		</c:if>
	</c:forEach>
	<!-- 分页逻辑结束 -->
	
	<a href="${pageContext.request.contextPath }/servlet/OrderList2Servlet?thispage=${requestScope.page.nextpage}">下一页</a>
	<a href="${pageContext.request.contextPath }/servlet/OrderList2Servlet?thispage=${requestScope.page.lastpage}">尾页</a>
	</div>  
  </body>
</html>
